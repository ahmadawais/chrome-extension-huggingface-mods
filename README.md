...
- **Model Tree sidebar** — when browsing quantized/finetuned models, the base model name in the sidebar gets a size badge so you know the source model's weight
...
`Largest size` / `Smallest size` are injected into HF's sort dropdown wherever model cards appear. The active option gets a ✓ and the sort button label updates to `Sort: Size ↓` / `Sort: Size ↑`.
...
- Sort by size is client-side only: HF has no server-side size sort, so the extension reorders the currently loaded page of results (~30 cards) using the already-fetched sizes. It re-applies automatically when HF re-renders the list (filtering, pagination), resets when you pick a native sort, and cards whose size failed to load sink to the end
